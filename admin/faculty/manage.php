...
class ManageFaculty extends BasePage
{
	...
	{
		$stmt = $this->getConnection()->prepare("CALL getFacultyByID(?)");
...
$page = new ManageFaculty();

if (isset($_GET['id']) && !empty($_GET['id'])) {
	$faculty = $page->getFaculty($_GET['id']);

	if (count($faculty) > 0) {
		foreach ($faculty[0] as $k => $v) {
			$$k = $v;
		}
	}
}
		...
		<div class="col-md-6">
			...
			<div class="form-group">
				...
				<select name="department_id" id="department_id" class="custom-select custom-select-sm select2" required="">
					<option value=""></option>
					<?php foreach ($page->getDepartments() as $k => $row) : ?>
